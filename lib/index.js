 ...
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync, renameSync, readdirSync, statSync, copyFileSync, unlinkSync } from 'node:fs'
...
export function apply(ctx, config) {
  ...
  function enqueueProposals(requests, sid, cwd) {
    ...
    for (const req of (requests || []).slice(0, 20)) {
      ...
      added.push(addProposal(queue, proposalsFile, { source: sid ? 'model' : 'api', track: built.ops[0].target, kind: built.ops[0].op, summary: req.summary, reason: req.reason || '', ops: built.ops, sessionId: sid || null, cwd: cwd || null }))
    }
