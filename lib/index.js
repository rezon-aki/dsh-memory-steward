...
function addProposal(queue, file, p) {
  ...
  proposeCharsBuf += JSON.stringify(item.ops).length
  saveJson(file, queue)
  return item
}
...
export function apply(ctx, config) {
  ...
  let proposeCharsBuf = 0   // 本轮提案正文（模型输出）字符数，挂到轮次上
  ...
  ctx.effect(() => ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      rounds.open.proposeChars = (rounds.open.proposeChars || 0) + proposeCharsBuf
      proposeCharsBuf = 0
      try { saveRounds() } catch { /* 静默 */ }
        ...
        handler: async (req, res) => {
          ...
          try {
            if (req.method === 'GET' && path === '/memory-steward/api/status') {
              refresh(cwdCache)
              return send(200, { ok: true, skill, clientPath: clientPathDebug(), rows, pending: pending().length, config: Object.assign(cfgPayload(), { fixedBudgets: cfg.budgets, reportOnly: cfg.reportOnly }), sweep: sweepState(), lastCheck, lastError, projects: snap ? snap.projects : [], clusters })
            }
            ...
            if (req.method === 'POST' && path === '/memory-steward/api/scan') {
              ...
              openRound(); rounds.open.auditCalls += 1; rounds.open.proposals += added.length; rounds.open.proposeChars = (rounds.open.proposeChars || 0) + proposeCharsBuf; proposeCharsBuf = 0; try { saveRounds() } catch { /* 静默 */ }
