 ...
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync, renameSync, readdirSync, statSync, copyFileSync } from 'node:fs'
...
function addProposal(queue, file, p) {
...
async function applyProposal(item, state) {
  const results = []
  for (const op of item.ops) {
    if (!op.match) { results.push({ op: op.op, ok: false, message: '缺少 match（未解析到整条正文）' }); break }
    try { results.push({ op: op.op, ok: true, message: '', result: await callEvo(state.baseUrl(), op, item.sessionId) }) }
...
function backupTracks(item, memoryDir, cfg) {
  ...
  try {
    ...
    for (const p of [f.memory, f.user, f.key]) if (p && existsSync(p)) copyFileSync(p, join(dir, p.split('/').pop()))
    return dir
  } catch { return null }
}

/** 确定性提案：只处理扫描器判定为 duplicate 的全局轨簇（归档较旧一条；archive 可移回，执行前另有整文件备份）。 */
async function deterministicProposals(memoryDir, existing) {
  const report = await runScanner(memoryDir)
  const out = []
  const seen = new Set(existing.map((i) => i.summary))
...
export function apply(ctx, config) {
  ...
  const cfgPayload = () => ({ autoApprove: cfg.autoApprove, nudge: cfg.nudge, autoCheck: cfg.autoCheck, lightMode: cfg.lightMode, budgetRatio: cfg.budgetRatio, sweepIntervalDays: cfg.sweepIntervalDays, archiveMaxEntries: cfg.archiveMaxEntries, roundKeep: cfg.roundKeep, baseline: cfg.baseline, budgets: budgetsOf(cfg) })
  function openRound() {
    const now = Date.now()
  ...
  function nudgeLine(cwd) {
    ...
    return head + '【整理到期】本回合收尾前跑**一轮完整整理**（一条流水线，主轨+归档一起过）：' + light + '② 主轨判 D/S/M/O/A/K——新事实照常写、重复 remove、被覆盖/过时/低价值 archive、同主题 replace 合并；③ 归档清运——已被主轨收录 / 被新事实推翻 / 一次性状态快照 → purge 删除；④ memory_propose 用 proposals 数组批量提交；⑤ memory_sweep_status complete 复位。用户随后在「整理审批」Tab 批量勾选采纳。'
  }
  ...
  async function autoApprovePending() {
    ...
    for (const item of targets) {
      ...
      const results = await applyProposal(item, { baseUrl })
  ...
  async function generateAndQueue(sessionId) {
    const added = []
    try {
      const props = await deterministicProposals(memoryDir, queue.items)
      for (const p of props) added.push(addProposal(queue, proposalsFile, { ...p, sessionId: sessionId || null, cwd: cwdCache }))
  ...
  ctx.effect(() => ctx.tools.register({
    ...
    async execute(args, exec) {
      const hdr = exec && exec.agent && exec.agent.session ? exec.agent.session.header : null
      const sid = exec && exec.agent && exec.agent.session ? exec.agent.session.id : null
      const cwd = hdr ? hdr.cwd : null
      const buildOps = (rawOps) => {
        ...
        for (const raw of rawOps || []) {
          ...
          ops.push(raw.op === 'replace'
            ? { op: 'replace', target: raw.target, match: r.entry, content: String(raw.content || '').trim() }
            : { op: raw.op, target: raw.target, match: r.entry })
        }
      ...
      for (const req of requests) {
        ...
        added.push(addProposal(queue, proposalsFile, { source: 'model', track: built.ops[0].target, kind: built.ops[0].op, summary: req.summary, reason: req.reason || '', ops: built.ops, sessionId: sid, cwd: cwd || null }))
      }
      if (added.length) { cfg.lastSweepAt = Date.now(); saveConfig() }
      openRound()
      ...
      return { ok: true, message: '已加入待审批队列 ' + added.length + ' 条：' + added.map((i) => i.id).join(',') + '（待审 ' + pending().length + ' 条）' + (auto.length ? '；已自动采纳 ' + auto.length + ' 条' : '') }
    },
        ...
        handler: async (req, res) => {
          ...
          try {
            if (req.method === 'GET' && path === '/memory-steward/api/status') {
              refresh(cwdCache)
              return send(200, { ok: true, clientPath: clientPathDebug(), rows, pending: pending().length, config: Object.assign(cfgPayload(), { fixedBudgets: cfg.budgets, reportOnly: cfg.reportOnly }), sweep: sweepState(), lastCheck, lastError, projects: snap ? snap.projects : [], clusters })
            }
            if (req.method === 'GET' && path === '/memory-steward/api/proposals') return send(200, { ok: true, items: queue.items.slice().reverse() })
            if (req.method === 'GET' && path === '/memory-steward/api/rounds') return send(200, { ok: true, open: rounds.open, rounds: rounds.rounds })
            if (req.method === 'POST' && path === '/memory-steward/api/scan') {
              const added = await generateAndQueue(null)
              openRound(); rounds.open.auditCalls += 1; rounds.open.proposals += added.length; try { saveRounds() } catch { /* 静默 */ }
            ...
            if (req.method === 'POST' && path === '/memory-steward/api/config') {
              ...
              saveConfig()
              return send(200, { ok: true, config: cfgPayload() })
            }
            ...
            if (req.method === 'POST' && m) {
              ...
              for (const item of queue.items.filter((i) => ids.has(i.id))) {
                ...
                } else if (action === 'reject') { item.status = 'rejected'; out.push({ id: item.id, ok: true, message: '已拒绝' }) }
                else { item.status = 'pending'; out.push({ id: item.id, ok: true, message: '已恢复待审' }) }
              }
              if (action === 'purge') { const ids = new Set((body.ids && body.ids.length) ? body.ids : queue.items.filter((i) => i.status !== 'pending').map((i) => i.id)); queue.items = queue.items.filter((i) => !ids.has(i.id)); out.length = 0; out.push({ id: '*', ok: true, message: '已清除 ' + ids.size + ' 条历史' }) }
