window.__ModuleLoader__.load({ id: "dsh-memory-steward", factory: (require) => {
...
const OP_LABEL = { archive: "归档", remove: "删除", purge: "清除归档", replace: "合并重写" };
const btnLink = { background: "none", border: "none", padding: "2px 0", color: "#0969da", cursor: "pointer", fontSize: 12 };
const preStyle = { whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 200, overflow: "auto", background: "var(--dsh-code-bg, rgba(127,127,127,0.10))", padding: "6px 8px", borderRadius: 6, fontSize: 12, margin: "2px 0 0", fontFamily: "inherit" };
...
function ProposalItem(props) {
  ...
  const d = props.detail || null;                       // 展开时按需拉取的完整条目：{loading} | {error} | {item}
  const full = d && d.item;
  const count = it.opCount != null ? it.opCount : ((full && full.ops) || []).length;
  return h("div", { style: Object.assign({}, card, { marginBottom: 8 }) },
    h("div", { style: row },
      ...
      h("strong", null, it.id),
      h("span", { style: muted }, it.track + " · " + it.kind + (it.opsBrief ? " · " + it.opsBrief : "")),
      it.source === "scan" ? h("span", { style: Object.assign({}, muted, { color: "#0969da" }) }, "确定性") : null,
      it.slim ? h("span", { style: Object.assign({}, muted, { color: "#bf8700" }) }, "正文已精简") : null,
    ...
    h("div", { style: { marginTop: 2 } },
      h("button", { style: btnLink, onClick: () => props.toggleOpen(it.id) },
        (props.open ? "▾ 收起" : "▸ 查看详细") + "（" + count + " 个操作）")),
    props.open
      ? h("div", { style: { marginTop: 4 } },
          d && d.loading ? h("div", { style: muted }, "加载详情…")
          : d && d.error ? h("div", { style: { color: "#cf222e", fontSize: 12 } }, "详情加载失败：" + d.error + "（收起后重新展开可重试）")
          : full
            ? [
                it.slim ? h("div", { key: "slim", style: Object.assign({}, muted, { color: "#bf8700" }) }, "正文已精简：执行前的整份文件在 steward/backups/" + it.id + "-<ts>/ 里，可据此还原") : null,
                full.reason && full.reason !== it.reason ? h("div", { key: "reason", style: muted }, "判定依据：" + full.reason) : null,
                full.evidence ? h("div", { key: "evidence", style: Object.assign({}, muted, { marginTop: 4 }) }, "证据：" + JSON.stringify(full.evidence)) : null,
                (full.ops || []).map((op, i) =>
                  h("div", { key: i, style: { borderLeft: "3px solid var(--dsh-border, #d0d7de)", paddingLeft: 8, marginBottom: 10 } },
                    h("div", { style: { fontSize: 12, fontWeight: 600, marginBottom: 2 } },
                      (i + 1) + ". " + (OP_LABEL[op.op] || op.op) + " · " + (TRACK_LABEL[op.target] || op.target) + (op.content ? "　（有改写内容）" : "")),
                    h("div", { style: muted }, op.op === "replace" ? "原记忆（将被新正文替换）" : "此条将从主轨移出，可在记忆 Tab 归档页移回"),
                    h("pre", { style: preStyle }, op.match || ""),
                    op.op === "replace" ? h("div", { style: { marginTop: 6 } }, h("div", { style: muted }, "修改后"), h("pre", { style: preStyle }, op.content || "")) : null)),
              ]
            : h("div", { style: muted }, "加载详情…"))
      : null,
    ...
    h("div", { style: Object.assign({}, row, { marginTop: 8 }) },
      ...
      it.status !== "pending" && !it.slim ? h("button", { style: btn, disabled: busy, onClick: () => props.act("restore", [it.id]) }, "恢复待审") : null,
      it.status !== "pending" ? h("button", { style: btnDanger, disabled: busy, onClick: () => props.act("purge", [it.id]) }, "删除记录") : null,
...
function Panel(props) {
  ...
  const [open, setOpen] = React.useState({});   // 展开查看详情的提案 id
  const [detail, setDetail] = React.useState({});   // id → {loading}|{error}|{item}：正文只在展开时按需拉
  const loadDetail = (id) => {
    setDetail((d) => Object.assign({}, d, { [id]: { loading: true } }));
    getJson(API + "/proposals/" + encodeURIComponent(id))
      .then((r) => setDetail((d) => Object.assign({}, d, { [id]: { item: r.item } })))
      .catch((e) => setDetail((d) => Object.assign({}, d, { [id]: { error: String(e.message || e) } })));
  };
  const toggleOpen = (id) => {
    const next = !open[id];
    setOpen((s) => Object.assign({}, s, { [id]: next }));
    if (next) loadDetail(id);
  };
  ...
  const act = async (action, ids) => {
    const list = ids || [];
    if (!list.length && action !== "purge") { setErr("没有选中任何提案"); return; }
    // 清除只删队列记录，不动记忆内容；清空全部历史是不可逆的批量动作，先确认
    if (action === "purge" && typeof window !== "undefined" && typeof window.confirm === "function") {
      const what = list.length ? (list.length + " 条记录") : "全部历史记录";
      if (!window.confirm("从队列删除 " + what + "？（只删记录，不影响记忆内容）")) return;
    }
    setBusy(list.length === 1 ? list[0] : ("bulk:" + list.length));
    try {
      const r = await post(API + "/proposals/" + action, { ids: list });
      setSel({});
      await load();
      const results = (r && r.results) || [];
      const okN = results.filter((x) => x.ok).length;
      const verb = { approve: "采纳", reject: "拒绝", restore: "恢复", purge: "清除" }[action] || "处理";
      setErr(results[0] && results[0].message ? results[0].message : (verb + " " + okN + "/" + list.length + " 条完成" + (okN < list.length ? "（失败项见下方列表）" : "")));
    } catch (e) { setErr(String(e.message || e)); }
    finally { setBusy(""); }
  };
  const scan = async () => {
  ...
  const clearLog = async (days) => {
    if (typeof window !== "undefined" && typeof window.confirm === "function") {
      const what = days > 0 ? ("删除 " + days + " 天前的日志行") : "清空全部事件日志（不可逆）";
      if (!window.confirm(what + "？只删 steward/history.jsonl，不动队列与记忆内容。")) return;
    }
    try { const r = await post(API + "/history/clear", days > 0 ? { days } : { all: true }); await load(); setErr("日志已清理：删除 " + r.removed + " 行，现存 " + ((r.log && r.log.lines) || 0) + " 行"); }
    catch (e) { setErr(String(e.message || e)); }
  };
  ...
  const hst = (status && status.history) || {};
  ...
  return h("div", { style: { padding: "10px 12px", overflow: "auto", height: "100%" } },
    ...
    h("div", { style: Object.assign({}, card, { marginTop: 10 }) },
      ...
      h("div", { style: Object.assign({}, row, { marginTop: 6 }) },
        h("span", { style: muted }, "历史保留"),
        h("input", { type: "number", min: 1, defaultValue: cfg.historyKeep || 20, style: { width: 46, fontSize: 12, padding: "2px 4px" }, onBlur: (e) => setCfg({ historyKeep: Number(e.target.value) || 20 }) }),
        h("span", { style: muted }, "条 ·"),
        h("input", { type: "number", min: 0, defaultValue: cfg.historyKeepDays === 0 ? 0 : (cfg.historyKeepDays || 30), style: { width: 46, fontSize: 12, padding: "2px 4px" }, onBlur: (e) => setCfg({ historyKeepDays: Math.max(0, Number(e.target.value) || 0) }) }),
        h("span", { style: muted }, "天（超出最近 N 条且早于此天数才淘汰；待审永不自动清）"),
        h("label", { style: { fontSize: 12, cursor: "pointer" } },
          h("input", { type: "checkbox", checked: cfg.historySlim !== false, onChange: (e) => setCfg({ historySlim: e.target.checked }) }), " 已结项精简正文（正文只留 backups）")),
    ...
    h("div", { style: Object.assign({}, card, { marginTop: 10 }) },
      h("div", { style: row },
        ...
        h("span", { style: muted }, "≈tokens = 盘点清单（输入）+ 提案正文（输出），按字符 ÷ 2 估")),
      roundSum && roundSum.count
        ? h("div", { style: { marginTop: 4, fontSize: 13 } },
            "最近 " + roundSum.count + " 轮平均 ≈" + fmtTokens(roundSum.avgTokens) + " tokens；上一轮 ≈" + fmtTokens(roundSum.lastTokens) + " tokens（盘点 " + fmtTokens(roundSum.lastAuditTokens) + " + 提案 " + fmtTokens(roundSum.lastProposeTokens) + "）")
        : null,
      openRound
        ? h("div", { style: Object.assign({}, muted, { marginTop: 4 }) }, "进行中 " + openRound.id + "：盘点 " + openRound.auditCalls + " 次 ≈" + fmtTokens(openRound.tokens) + " tokens（盘点 " + fmtTokens(openRound.auditTokens) + " + 提案 " + fmtTokens(openRound.proposeTokens) + "） · 提案 " + openRound.proposals + " 条/" + openRound.ops + " ops · 已执行 " + openRound.applied + "（失败 " + openRound.failed + "）")
        : null,
      ...
      rounds.length
        ? h("table", { style: { borderCollapse: "collapse", width: "100%", marginTop: 4 } },
            ...
            h("tbody", null, rounds.map((r) => h("tr", { key: r.id },
              ...
              h("td", { style: Object.assign({}, td, { title: "盘点 ≈" + fmtTokens(r.auditTokens) + " + 提案 ≈" + fmtTokens(r.proposeTokens) }) }, fmtTokens(r.tokens)),
    ...
    h("div", { style: Object.assign({}, card, { marginTop: 10 }) },
      h("div", { style: Object.assign({}, row, { justifyContent: "space-between" }) },
        ...
        tab === "pending" && pending.length > 1
          ...
          : (tab === "history" && history.length > 0 ? h("button", { style: Object.assign({}, btn, busy ? { opacity: 0.5 } : {}), disabled: !!busy, onClick: () => act("purge", []) }, "清空历史（只删记录）") : null)),
      tab === "pending" && pending.length > 0
        ? h("div", { style: Object.assign({}, row, { marginTop: 8 }) },
            h("span", { style: muted }, "批量："),
            h("button", { style: btn, disabled: !!busy, onClick: selectAll }, "全选 (" + pending.length + ")"),
            h("button", { style: btn, disabled: !!busy, onClick: invert }, "反选"),
            h("button", { style: btn, disabled: selIds.length === 0 || !!busy, onClick: clearSel }, "清除选择"),
            h("button", { style: Object.assign({}, btnPrimary, (selIds.length === 0 || busy) ? { opacity: 0.5 } : {}) , disabled: selIds.length === 0 || !!busy, onClick: () => act("approve", selIds) }, busy && String(busy).startsWith("bulk") ? "执行中…" : ("采纳选中 (" + selIds.length + ")")),
            h("button", { style: Object.assign({}, btnDanger, (selIds.length === 0 || busy) ? { opacity: 0.5 } : {}), disabled: selIds.length === 0 || !!busy, onClick: () => act("reject", selIds) }, "拒绝选中 (" + selIds.length + ")"))
        : null,
      tab === "history"
        ? h("div", { style: Object.assign({}, row, { marginTop: 8 }) },
            h("span", { style: muted }, "保留策略：最近 " + (hst.keep || 20) + " 条 · " + (hst.keepDays == null ? 30 : hst.keepDays) + " 天（超出且超期才淘汰）　已淘汰 " + (hst.pruned || 0) + " 条 · 已精简 " + (hst.slimmed || 0) + " 条　事件日志 steward/history.jsonl：" + ((hst.log && hst.log.lines) || 0) + " 行 / " + fmtBytes((hst.log && hst.log.bytes) || 0) + "（只记摘要与失败原因，不含正文）"),
            h("button", { style: btn, disabled: !!busy, onClick: () => clearLog(7) }, "清理日志（留 7 天）"),
            h("button", { style: btnDanger, disabled: !!busy, onClick: () => clearLog(0) }, "清空日志"))
        : null,
      h("div", { style: { marginTop: 8 } },
        (tab === "pending" ? pending : history).map((it) => h(ProposalItem, { key: it.id, item: it, act, busy, selectable: tab === "pending", checked: !!sel[it.id], toggle, open: !!open[it.id], toggleOpen, detail: detail[it.id] })),
