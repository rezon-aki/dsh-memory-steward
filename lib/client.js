...
function Panel(props) {
  ...
  const load = React.useCallback(async () => {
    try {
      ...
      setStatus(s); setItems(p.items || []); setRounds(rd.rounds || []); setOpenRound(rd.open || null); setErr(""); syncBadge(s && s.pending);
  ...
  return h("div", { style: { padding: "10px 12px", overflow: "auto", height: "100%" } },
    ...
    h("div", { style: Object.assign({}, card, { marginTop: 10 }) },
      h("div", { style: row },
        ...
        h("span", { style: muted }, "盘点字符 ≈ 该轮塞进上下文的清单量（粗估 tokens ≈ 字符/2）")),
      openRound
        ? h("div", { style: Object.assign({}, muted, { marginTop: 4 }) }, "进行中 " + openRound.id + "：盘点 " + openRound.auditCalls + " 次/" + openRound.auditChars + " 字符 · 提案 " + openRound.proposals + " 条/" + openRound.ops + " ops · 已执行 " + openRound.applied + "（失败 " + openRound.failed + "）")
        : null,
      ...
      rounds.length
        ? h("table", { style: { borderCollapse: "collapse", width: "100%", marginTop: 4 } },
            h("thead", null, h("tr", null,
              h("th", { style: th }, "开始"), h("th", { style: th }, "盘点"), h("th", { style: th }, "字符"),
            ...
            h("tbody", null, rounds.map((r) => h("tr", { key: r.id },
              ...
              h("td", { style: td }, String(r.auditChars)),
              h("td", { style: td }, r.proposals + "/" + r.ops),
              h("td", { style: td }, r.applied + (r.failed ? "（败 " + r.failed + "）" : "")),
              h("td", { style: Object.assign({}, td, muted) }, r.reason || "")))))
        : null),
    h("div", { style: Object.assign({}, card, { marginTop: 10 }) },
      h("div", { style: Object.assign({}, row, { justifyContent: "space-between" }) },
        h("div", { style: row },
