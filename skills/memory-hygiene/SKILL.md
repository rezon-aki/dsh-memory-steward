...
- key 的 `[summary:…]` 是**头部程序元数据 tag，编辑/合并都不会更新它**（`update` 只重写正文、tag 原样保留）；只有「本来就没有 tag」的条目才按正文首行自动生成摘要。所以合并后旧摘要会一直挂着（可能描述已被推翻的状态）——要修正只有归档旧条目 + 用 `memory` 工具带 `summary` 重建，正文逐字搬运并 diff 核对。
- key 条目带 `[branch:…]` 时合并必须保留原分支范围，否则会变成「全部分支可见」，跨分支串味。
- 单轮动作 ≤20 条：先跑低风险（归档、完全重复删除、归档清运），合并类可单列一轮，避免一次改太多难核对。
- **批量写入后读盘要等一拍**：连续多次 archive/add（实测 8 连写）后立刻读文件可能落后于工具视图（archive 已落、add 未落），十几秒后才齐；单次写入是立即落盘的。用文件核对前先确认条数对得上，对不上稍等再读——不是写丢了。
