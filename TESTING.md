...
`npm test`（15 例）能覆盖的纯逻辑与 HTTP 层不在这里重复。这份只写**只有人或真环境才能验的**四类，每类都给了可复制的命令、期望结果和"不对时看哪里"。
...
2. `npm test` —— 15 例纯逻辑回归（若内核变了 API 形状，测试里的假环境也会先暴露一半）
...
- **备份是否在长**：采纳 N 次后 `backups/` 应有约 N 个目录；不涨说明 `backupOnApply` 失效
